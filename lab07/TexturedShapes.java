...
public class TexturedShapes {
    ...
    public void Triangle (GL2 gl2, float a, float b, float c, float d, float y)
	{
		...
		gl2.glNormal3f(a, -0.5f, b);
		gl2.glTexCoord3d(a, -0.5f, b);
		gl2.glVertex3f(a, -0.5f, b);
		
		gl2.glNormal3f(0, y, 0);
		gl2.glTexCoord3d(0,y,0);
		gl2.glVertex3f(0,y,0);
		
		
		gl2.glNormal3f(c, -0.5f, d);
		gl2.glTexCoord3d(c, -0.5f, d);
		gl2.glVertex3f(c, -0.5f, d);
		gl2.glEnd();
	...
	for (int i = 0; i<N; i++)
	{
	 x[i] = (float) Math.sin(i * (2 * Math.PI)/N);
	 z[i] = (float) Math.cos(i * (2 * Math.PI)/N);
	...
	{
	gl2.glPushMatrix();
	Triangle(gl2, x[i-1], z[i-1], x[i], z[i], -0.5f);
	Triangle(gl2, x[i-1], z[i-1], x[i], z[i], 1);	
	gl2.glPopMatrix();
	}
	
	
	gl2.glPushMatrix();
	Triangle(gl2, x[N-1], z[N-1], x[0], z[0], -0.5f);
	Triangle(gl2, x[N-1], z[N-1], x[0], z[0], 1);
	gl2.glPopMatrix();
	}
