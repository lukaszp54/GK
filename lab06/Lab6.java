...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void Triangle (GL2 gl2, float a, float b, float c, float d, float y)
	{
		...
		gl2.glNormal3f(a, -3, b);
		gl2.glVertex3f(a, -3, b);
		gl2.glNormal3f(0, y, 0);
		gl2.glVertex3f(0,y,0);
		gl2.glNormal3f(c, -3, d);
	...
	{	
	gl2.glPushMatrix();
	gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[i+2], i);
	Triangle(gl2, x[i-1], z[i-1], x[i], z[i], -3);
	Triangle(gl2, x[i-1], z[i-1], x[i], z[i], 4);	
	gl2.glPopMatrix();
	}
	
	
	gl2.glPushMatrix();
	gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[3], 9);
	Triangle(gl2, x[N-1], z[N-1], x[0], z[0], -3);
	Triangle(gl2, x[N-1], z[N-1], x[0], z[0], 4);
	gl2.glPopMatrix();
	}
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl2.glLightfv(GL2.GL_LIGHT0, GL2.GL_POSITION, new float[]{2,2,0,0},0);
		gl2.glLightfv(GL2.GL_LIGHT1, GL2.GL_POSITION, new float[]{-2,2,0,0},0);
		gl2.glLightfv(GL2.GL_LIGHT2, GL2.GL_POSITION, new float[]{0,2,2,0},0);
		gl2.glLightfv(GL2.GL_LIGHT3, GL2.GL_POSITION, new float[]{0,2,-2,0},0);
